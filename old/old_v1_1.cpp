	...
	// Aplicar la lógica del clic del ratón si está presionado
        ...
        for (size_t i = 0; i < points.size(); ++i) {
		    ...
		    if(points[i].color.g > 0){
                    ...
                    // Reduzco la velocidad de los puntos en caso de choque simulando perdida de energía cinética
                    // points[i].xVelocity = points[i].xVelocity/2;
                    // points[j].xVelocity = points[j].xVelocity/2;
                    // points[i].yVelocity = points[i].yVelocity/2;
                    // points[j].yVelocity = points[j].yVelocity/2;
